...
#
# Installs: ~/.local/bin/{thub,tside,twin,twt-all,tmux-restore-if-empty,tmux-save-guarded[,ccr]}, fzf, TPM + resurrect +
...
for f in thub tside twin twt-all tmux-restore-if-empty tmux-save-guarded; do install -m755 "$HERE/bin/$f" ~/.local/bin/; done
